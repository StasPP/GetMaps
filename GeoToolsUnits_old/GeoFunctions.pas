...
uses GeoClasses, GeoMath, Math;
  ...
  const
       SuccessStr = ' Перевод успешно выполнен!';
...
begin
  try
    ...
    L2 := 0;
    H2 := 0;
    Result := 'Не найдены системы координат!';
    exit;
  end;
  ...
  begin
    ...
    L2 := 0;
    H2 := 0;
    Result := 'Не найдены параметры перехода!';
    exit;
  end;
...
begin
  ...
  try
    Result := 'Выполняю: ' + DatumList[InputSKNumber].Name;
  except
    Result := 'Исходная СК не найдена!';
    exit;
  end;
 ...
 // ShortestIsBad :=false;
  ...
  begin
    ...
    begin
       ...
       begin
        ...
        H1 := H2;
          ...
          begin
            Result := Result  + '. Ошибка перевода к: ' + DatumList[GeoDatums[i]].Name;
            exit;
          end;
    end;
    Result := Result + SuccessStr;
      ...
      Result := 'Параметры перехода не найдены';
