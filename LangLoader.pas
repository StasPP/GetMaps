...
interface
 ...
 uses SysUtils, Forms, Classes, StdCtrls, ExtCtrls, Buttons, Menus, ComCtrls, GeoFiles;
...
begin
  ...
  S.SaveToFile('Data\Mapper\Default.txt');
  S.Free;
...
begin
  ...
  S.LoadFromFile('Data\Mapper\'+Lang+'\Main.txt');
  ...
  S.LoadFromFile('Data\Mapper\'+Lang+'\Info.txt');
  for I := 0 to 255 do
  Begin
    if I <= S.Count-1 then
  ...
  S.LoadFromFile('Data\Mapper\'+Lang+'\Locs.txt');

  if OldLang<>Lang then
    GeoTranslate(OldLang, Lang, 'Data\Mapper\');
