...
uses SysUtils, Classes, ExtCtrls, Windows, Dialogs, Graphics, ComCtrls,
  ...
  function CropImageToFiles256(Image:TImage; SaveDir, FName:string;
                            SaveFirstOnly: Boolean): integer; /// returns the cropped images count
...
begin
  ...
  if X mod 256 <> 0 then
     ExtendImg.Width := trunc(X/256 + 1)*256;

  if Y mod 256 <> 0 then
     ExtendImg.Height := trunc(Y/256 + 1)*256;

  NX := trunc(ExtendImg.Width/256);
  NY := trunc(ExtendImg.Height/256);
  ...
  for i := 0 to NX -1 do
    for j := 0 to NY -1 do
     begin
       CropImg.Canvas.CopyRect(Rect(0,0,256,256),
                               ExtendImg.Canvas, Rect(i*256, j*256, (i+1)*256, (j+1)*256));
...
begin
  ...
  Begin
    ...
    Str[2] := s+'.jpg';
    for K := 0 to 3 do
       Str[50+K] := Str[50+K] + GetCols(Str2[0],3+2*K,1,' ', false)
                    + ', '+ GetCols(Str2[0],2+2*K,1,' ', false);

    s2 := '';
    for K := 0 to 3 do
    Begin
      B  := StrToFloat2(GetCols(Str2[0],2+K*2,1,' ', false));
      j  := Abs(Trunc(B));
      ...
      L := StrToFloat2(GetCols(Str2[0],3+K*2,1,' ', false));
      j := Abs(Trunc(L));
      s2 := s2 + ' '+ IntToStr(J)+', '+ format('%.12f',[Abs(L-j)*60]);
